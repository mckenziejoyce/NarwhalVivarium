...
public class Component implements Rotatable, Nameable, Colorable, UpdatingDisplayable {
	...
	//A boolean to determine if the narwhal changed its X direction 
	public boolean changeDirectionXNar;
	//A boolean to determine if the narwhal changed its Y direction 
	public boolean changeDirectionYNar;
	//A boolean to determine if the narwhal changed its Z direction 
	public boolean changeDirectionZNar;
	//A boolean to determine if the fish changed its X direction 
	public boolean changeDirectionXFish;
	//A boolean to determine if the fish changed its Y direction 
	public boolean changeDirectionYFish;
	//A boolean to determine if the fish changed its Z direction 
	public boolean changeDirectionZFish;
	...
	public Component(final Point3D position, final String name) {
		...
		this.changeDirectionXNar = false;
		this.changeDirectionYNar = false;
		this.changeDirectionZNar = false;
		this.changeDirectionXFish = false;
		this.changeDirectionYFish = false;
		this.changeDirectionZFish = false;
	}
	...
	//Another rotation function that takes in the axis of rotation which can have values (inc partial) for X,Y,andZ axis instead of only a full rotation on one axis 
	public void rotate(float[] axis, final double angleDelta){
		// Use the value of the correspondng axis to determine how much of the angleDelta to apply to each Axis 
		double xRatio = axis[0] * angleDelta;
		double yRatio = axis[1] * angleDelta;
		double zRatio = axis[2] * angleDelta;
		//Change the angle 
		this.xAngle += xRatio;
		this.yAngle += yRatio;
		this.zAngle += zRatio;
		...
		//Change in X direction for narwhal 
		if(this.changeDirectionXNar == true && this.changeDirectionYNar == false) {
			this.yAngle += 180;
			this.changeDirectionXNar = false;
		}
		//Change in Y direction for narwhal 
		else if(this.changeDirectionYNar == true && this.changeDirectionXNar == false) {
			this.xAngle += 180;
			this.changeDirectionYNar = false;
		}
		//Change in X and Y direction for narwhal 
		else if(this.changeDirectionYNar == true && this.changeDirectionXNar == true){
			this.zAngle += 180;
			this.changeDirectionXNar = false;
			this.changeDirectionYNar = false;
		}
		else if(this.changeDirectionXFish == true && this.changeDirectionYFish == false) {
			//System.out.println("change X direction");
			this.yAngle += 180;
			this.changeDirectionXFish = false;
		}
		else if(this.changeDirectionXFish == false && this.changeDirectionYFish == true) {
			//System.out.println("change y direction");
			this.xAngle += 180;
			this.changeDirectionYFish = false;
		}
		else if(this.changeDirectionXFish == true && this.changeDirectionYFish == true ) {
			//System.out.println("change z direction");
			this.zAngle += 180;
			this.changeDirectionXFish = false;
			this.changeDirectionYFish = false;
		}
...
////Change in Z direction for narwhal 
////if(this.changeDirectionXNar == false && this.changeDirectionYNar == false && this.changeDirectionZNar == true) {
////this.yAngle += 90;
////this.xAngle += 90;
////this.changeDirectionZNar = false;
////}
...
//if(this.changeDirectionYNar == true && this.changeDirectionXNar == false) {
//double xPrimeX = this.xAngle;
//double yPrimeX = this.yAngle * Math.cos(180) - this.zAngle * Math.sin(180);
//double zPrimeX = this.yAngle * Math.sin(180) + this.zAngle * Math.cos(180);
//this.xAngle += xPrimeX;
//this.yAngle += yPrimeX;
//this.zAngle += zPrimeX;
//this.changeDirectionYNar = false;
//}
//if(this.changeDirectionXNar == true && this.changeDirectionYNar == false) {
//double xPrimeY = this.zAngle * Math.sin(180) + this.xAngle * Math.cos(180);
//double yPrimeY = this.yAngle;
//double zPrimeY = this.zAngle * Math.cos(180) - this.xAngle * Math.sin(180);
//this.xAngle += xPrimeY;
//this.yAngle += yPrimeY;
//this.zAngle += zPrimeY;
//this.changeDirectionXNar = false;
//}
//if(this.changeDirectionZNar == true) {
//double xPrimeZ = this.xAngle * Math.cos(180) - this.yAngle * Math.sin(180);
//double yPrimeZ = this.xAngle * Math.sin(180) + this.yAngle * Math.cos(180);
//double zPrimeZ = this.zAngle;
//this.xAngle += xPrimeZ;
//this.yAngle += yPrimeZ;
//this.zAngle += zPrimeZ;
//this.changeDirectionZNar = false;
